...
1. 通用性强
   - 支持所有文件格式的编解码
   - 最大支持约 122.189GB 的文件处理
   - 每帧载荷 1320 有效字节
   - 4×2 格式传输速率达到 379.68KB/s  (每秒30帧)
   - 2×1 格式传输速率达到 94.92KB/s (每秒30帧)
   - 如果提升帧率，传输速率将进一步提升，取决于min(f<sub>摄像头</sub>, f<sub>显示器</sub>)

2. 高效并行处理
   - 采用多进程并行解码技术
   - 自动检测CPU核心数量
   - 动态分配计算资源
   - 解码速度提升8-16倍（取决于CPU核心数）
